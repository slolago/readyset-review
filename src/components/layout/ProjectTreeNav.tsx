...
export function ProjectTreeNav() {
  ...
  return (
    <div className="overflow-y-auto flex-1 px-1 pb-4">
      <p className="text-xs font-semibold text-scope-textMuted uppercase tracking-wider px-2 mb-1.5 mt-3">
        Projects
      </p>
          ...
          <div key={project.id} className="mb-0.5">
            ...
            <div
              className={cn(
                ...
                isProjectSelected &&
                  'before:absolute before:left-0 before:top-1 before:bottom-1 before:w-0.5 before:bg-scope-accent before:rounded-r',
                isProjectParentOfSelected &&
                  'before:absolute before:left-0 before:top-1 before:bottom-1 before:w-0.5 before:bg-scope-accent/40 before:rounded-r'
              )}
            >
              <button
              ...
              >
                <ChevronRight
                  className={cn(
                    'w-3.5 h-3.5 text-scope-textMuted transition-transform duration-150',
                    expanded && 'rotate-90'
                  )}
                />
              </button>
              ...
              <Link
                ...
                className={cn(
                  ...
                  isProjectSelected
                    ? 'bg-scope-accent/15 text-white font-medium'
                    : isProjectParentOfSelected
                    ? 'text-white hover:bg-white/5'
                    : 'text-scope-textSecondary hover:bg-white/5 hover:text-white'
                )}
              >
                {project.name}
              </Link>
            ...
            {/* Folder rows */}
            {expanded && (
              <div className="ml-3 border-l border-scope-border/40 pl-1 mt-0.5 space-y-0.5">
                {!foldersLoaded ? (
                  <div
                    className="w-3 h-3 border border-scope-accent/40 border-t-scope-accent rounded-full animate-spin ml-3 my-1"
                    aria-label="Loading folders"
                  />
                ) : folders.length === 0 ? (
                  <span className="pl-3 text-xs text-scope-textMuted/70 italic">
                    No folders
                  </span>
                ) : (
                  folders.map((folder) => {
                  ...
                  <Link
                    ...
                    className={cn(
                      ...
                      pathname.startsWith(`/projects/${project.id}/review-links`)
                        ? 'bg-scope-accent/15 text-white font-medium'
                        : 'text-scope-textMuted hover:bg-white/5 hover:text-white'
                    )}
                  >
